...
Working on how this might improve current dictionary coders (or more like whether it can beat them at all), I figured [patterns](#patterns) are possible.
...
$W = \{w : \|w\| < m\}$  
Next we rank each word using a [ranking](#ranking) algorithm: $r(w) \in \R, \forall w\in W$.
...
$$
\gamma = \log_{\alpha}d \times {{1}\over{\sum_{w \in W} p(w)\|w\|}}
$$
For an alphabet $A: \|A\| = \alpha$, dictionary of size $d$ and probability of word $p(w): \sum_{w \in W} p(w) = 1$  
...
### <a name="context"></a> 1b. Context splitting
...
$S_1 \cup S_2 = A, S_1 \cap S_2 = \emptyset$  
